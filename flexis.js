document.addEventListener('DOMContentLoaded', () => {
  const { startGame, gameIsOver } = (() => {
    ...
    board.appendChild(boxesContainer);
    ...
    const stopDrop = () => { clearInterval(dropInterval); clearTimeout(dropTimeout); };
    ...
    const flashLines = lines => {
      ...
      setTimeout(() => { flashEmpty(); shuffleLines(); flashing = false; dropInterval = startDrop(); }, 1250);
    };
    ...
    const startDrop = (time = 500) => dropInterval = setInterval(drop, time);
    ...
    document.addEventListener('keydown', e => {
      ...
      switch (e.keyCode) {
        ...
        case 38:
          ...
          break;
        case 40:
          if(!droppingFast && !flashing) {
            droppingFast = true;
            stopDrop();
            dropTimeout = setTimeout(() => { drop(); startDrop(100); }, Math.max(0, 100 - (new Date() - lastDropTime)));
          }
          break;
      }
    });
    const gameIsOver = () => !gameRunning;
    document.addEventListener('keyup', e => {
      if (e.keyCode === 40 && droppingFast && !flashing) {
        droppingFast = false;
        stopDrop();
        dropTimeout = setTimeout(() => { drop(); startDrop(); }, Math.max(0, 500 - (new Date() - lastDropTime)));
      }
    });
    return { startGame, gameIsOver };
  })();
  let upPressedTimes = 0;
  document.addEventListener('keydown', e => {
    if (e.keyCode === 38 && gameIsOver()) {
      upPressedTimes++;
      if(upPressedTimes === 3) {
        upPressedTimes = 0;
        startGame();
